...
class dataset(object):
    def __init__(self,filename,opt):
        ...
        for line in tqdm(f):
            ...
            cases, rev_count, intro_count, intro_entity_len =self._context_reformulate(contexts,movies,altitude,initial_altitude,seekerid,recommenderid)
            total_rev_count += rev_count
            total_intro_count += intro_count
            total_intro_count += intro_count
            total_intro_entity += intro_entity_len
            self.data.extend(cases)
        print("total_rev_count:",total_rev_count,", total_intro_count:",total_intro_count, ", total_intro_entity:",total_intro_entity)
    ...
    def data_process(self,is_finetune=False):
        ...
        for line in self.data:
            ...
            data_set.append([np.array(context),c_lengths,np.array(response),r_length,np.array(mask_response),mask_r_length,line['entity'],line['entities_altitude'],
    ...
    def detect_movie(self,sentence, movies):
        ...
        for movie in movie_rec:
            ...
            try:
                ...
                pass
        return token_text_com, movie_rec_trans, rev_count, intro_count, list(set(entities))
    ...
    def _context_reformulate(self,context,movies,altitude,ini_altitude,s_id,re_id):
        ...
        for message in context:
            ...
            try:
                ...
                pass
            token_text, movie_rec, temp_rev_count, temp_intro_count, intro_entity = self.detect_movie(message['text'], movies)
        ...
        return cases,rev_count,intro_count, intro_entity_len
...
class CRSdataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        '''
        context, c_lengths, response, r_length, mask_response, mask_r_length, entity,entities_altitude, movie, movie_altitude, concept_mask, dbpedia_mask, reviews_mask, introduction_mask, rec = self.data[index]
        ...
        return context, c_lengths, response, r_length, mask_response, mask_r_length, entity_vec, entity_vector, entities_altitude_vec, movie, movie_altitude,np.array(concept_mask), np.array(dbpedia_mask), np.array(reviews_mask), np.array(introduction_mask), concept_vec, db_vec, rec
