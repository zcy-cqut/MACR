...
from dataset_intro import dataset,CRSdataset
...
class TrainLoop_fusion_rec():
    ...
    def train(self):
        ...
        for i in range(3):
            ...
            train_dataset_loader = torch.utils.data.DataLoader(dataset=train_set,batch_size=self.batch_size,shuffle=False)
            num=0
            for context,c_lengths,response,r_length,mask_response,mask_r_length,entity,entity_vector,entities_altitude,movie, movie_altitude, concept_mask,dbpedia_mask,reviews_mask,introduction_mask,concept_vec, db_vec,rec in tqdm(train_dataset_loader):
                seed_sets = []
                ...
                scores, preds, rec_scores, rec_loss, gen_loss, mask_loss, info_db_loss, _, info_genre_loss, genre_emb_count=self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, reviews_mask,introduction_mask,seed_sets, entities_sets_altitude, movie, movie_altitude, concept_vec, db_vec, entity_vector.cuda(), rec, test=False)
        ...
        for i in range(self.epoch):
            ...
            for context,c_lengths,response,r_length,mask_response,mask_r_length,entity,entity_vector,entities_altitude,movie,movie_altitude,concept_mask,dbpedia_mask,reviews_mask,introduction_mask,concept_vec,db_vec,rec in tqdm(train_dataset_loader):
                ...
                self.update_params()
                if num%50==0:
                    print('rec-loss:%f'%(sum([l[0] for l in losses])/len(losses)),', info-db-loss:%f'%(sum([l[1] for l in losses])/len(losses))) # ,', info-genre-loss:%f'%(sum([l[2] for l in losses])/len(losses))
                    losses=[]
                num+=1
            print("Number of categories used:", genre_emb_count)
    ...
    def val(self, is_test=False):
        ...
        recs=[]
        total_genre_emb_count = 0
        for context, c_lengths, response, r_length, mask_response, mask_r_length, entity, entity_vector,entities_altitude, movie, movie_altitude, concept_mask, dbpedia_mask,reviews_mask, introduction_mask, concept_vec, db_vec, rec in tqdm(val_dataset_loader):
            with torch.no_grad():
                seed_sets = []
                entities_sets_altitude = []
                batch_size = context.shape[0]
                for b in range(batch_size):
                    ...
                    seed_sets.append(seed_set)
                    entities_sets_altitude.append(entities_altitude[b])
                scores, preds, rec_scores, rec_loss, _, mask_loss, info_db_loss, info_con_loss,info_genre_loss,genre_emb_count= self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask,reviews_mask, introduction_mask, seed_sets, entities_sets_altitude, movie, movie_altitude, concept_vec, db_vec, entity_vector.cuda(), rec, test=True, maxlen=20, bsz=batch_size)
                total_genre_emb_count += genre_emb_count
            recs.extend(rec.cpu())
    ...
    def train(self):
        ...
        for i in range(self.epoch*3):
            ...
            for context, c_lengths, response, r_length, mask_response, mask_r_length, entity,entity_vector,entities_altitude, movie, movie_altitude, concept_mask, dbpedia_mask, reviews_mask, introduction_mask, concept_vec, db_vec, rec in tqdm(train_dataset_loader):
                ...
                scores, preds, rec_scores, rec_loss, gen_loss, mask_loss, info_db_loss, info_con_loss,info_genre_loss,genre_emb_count=self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, reviews_mask,introduction_mask, seed_sets, entities_altitude,movie, movie_altitude, concept_vec, db_vec, entity_vector.cuda(), rec, test=False)
        ...
        for context, c_lengths, response, r_length, mask_response, mask_r_length, entity, entity_vector,entities_altitude,  movie, movie_altitude, concept_mask, dbpedia_mask, reviews_mask,introduction_mask, concept_vec, db_vec, rec in tqdm(val_dataset_loader):
            ...
            with torch.no_grad():
                ...
                for b in range(batch_size):
                    ...
                    seed_sets.append(seed_set)
                    entities_sets_altitude.append(entities_altitude[b])
                _, _, _, _, gen_loss, mask_loss, info_db_loss, info_con_loss, info_genre_loss,genre_emb_count= self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, reviews_mask,introduction_mask, seed_sets,entities_sets_altitude,  movie, movie_altitude, concept_vec, db_vec, entity_vector.cuda(), rec, test=False)
                scores, preds, rec_scores, rec_loss, _, mask_loss, info_db_loss, info_con_loss, info_genre_loss,genre_emb_count= self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, reviews_mask,introduction_mask, seed_sets,entities_sets_altitude, movie, movie_altitude, concept_vec, db_vec, entity_vector.cuda(), rec, test=True, maxlen=50, bsz=batch_size)
